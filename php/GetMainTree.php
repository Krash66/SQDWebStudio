...
function getLevelFromDB($parent_id,$cnn){
	...
	$sql = "SELECT id, projName, objid, parentId, text, im0, im1, im2, sqdType, loadFunction, Icon, tabId FROM maintree WHERE  parentId='".$parent_id."' and projName='".$_POST["projName"]."' ORDER BY id";
...
echo "<tree id='fldProj'>\n";
//Call function to print tree XML
getLevelFromDB('fldProj',$conn);
//Close db connection                                                                                                                                               
mysqli_close($conn);
//Close Tree Tag
echo "</tree>";
